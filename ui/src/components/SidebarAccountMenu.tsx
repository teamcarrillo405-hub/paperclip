...
export function SidebarAccountMenu({
  ...
  const { productName, docsUrl } = useBrand();
    ...
    <div className="border-t border-r border-border bg-background px-3 py-2">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <button
            type="button"
            className="flex w-full items-center gap-2.5 px-3 py-2 text-left text-sm font-medium text-foreground/80 transition-colors hover:bg-accent/50 hover:text-foreground"
          ...
          <div className="-mt-8 px-4 pb-4">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1 pt-1">
                ...
                {version ? (
                  <p className="mt-1 text-xs text-muted-foreground">{productName} v{version}</p>
                ) : null}
              </div>
            </div>
            ...
            <div className="mt-4 space-y-1">
              ...
              <MenuAction
                label="Documentation"
                description={`Open ${productName} docs in a new tab.`}
                icon={BookOpen}
